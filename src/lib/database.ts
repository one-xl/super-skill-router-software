...
export async function recordLocalInstallations(skillName: string, directoryName: string, report: BatchInstallReport) {
  const database = await Database.load(DATABASE_URL);
  const now = new Date().toISOString();
  for (const result of report.results) {
    if (!result.outcome) continue;
    ...
    await database.execute(
      `INSERT INTO installation_records
        (skill_name, directory_name, repository, source_url, commit_sha, target, status, installed_path, package_path, installed_at, updated_at)
       VALUES ($1, $2, 'local-archive', '', '', $3, $4, $5, $6, $7, $8)
       ON CONFLICT(directory_name, target) DO UPDATE SET
         skill_name = excluded.skill_name, repository = excluded.repository, source_url = excluded.source_url,
         commit_sha = excluded.commit_sha, status = excluded.status, installed_path = excluded.installed_path,
         package_path = excluded.package_path, updated_at = excluded.updated_at`,
      [skillName, directoryName, result.target, status, result.outcome.path ?? null, result.outcome.zip_path ?? null, now, now],
    );
  }
}
