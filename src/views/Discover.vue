  ...
  <section class="mx-auto w-full max-w-6xl px-6 py-8 lg:px-10">
    <div class="mb-8 flex flex-wrap items-end justify-between gap-4">
      <div>
        ...
        <p class="mt-2 text-sm text-slate-500">从本地静态索引中找到适合当前工作的 skill。</p>
      </div>
      <button
        class="inline-flex h-10 items-center gap-2 rounded-lg border border-slate-200 bg-white px-3 text-sm font-medium text-slate-600 shadow-sm transition hover:border-teal-300 hover:text-teal-700 disabled:cursor-not-allowed disabled:opacity-50"
        type="button"
        :disabled="store.loading"
        title="重新加载索引"
        @click="store.load"
      >
        <RefreshCw class="size-4" :class="store.loading && 'animate-spin'" />
        刷新索引
      </button>
    </div>
    ...
    <div class="mt-5 flex flex-wrap items-center gap-2">
      ...
      <button
        ...
        :class="store.activeTags.includes(tag) ? 'border-teal-600 bg-teal-600 text-white' : 'border-slate-200 bg-white text-slate-500 hover:border-teal-300 hover:text-teal-700'"
    ...
    <div v-if="store.error" class="mt-5 border-l-4 border-amber-400 bg-amber-50 px-4 py-3 text-sm text-amber-900" role="status">
      {{ store.error }}
    </div>

    <div v-if="store.loading && !store.index" class="mt-8 space-y-5" aria-live="polite">
      <div v-for="n in 3" :key="n" class="h-36 animate-pulse rounded-xl bg-slate-200/70" />
    </div>
    <div v-else-if="!store.error && store.results.length === 0" class="mt-8 rounded-xl border border-dashed border-slate-300 bg-white px-6 py-16 text-center">
      <h2 class="text-base font-semibold text-slate-800">没有匹配的 skill</h2>
      <p class="mt-2 text-sm text-slate-500">尝试更短的关键词，或清除标签筛选。</p>
    </div>
