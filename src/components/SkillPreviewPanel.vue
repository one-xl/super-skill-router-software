...
<template>
  <div class="min-w-0 max-w-full overflow-hidden border-l-2 border-teal-600 bg-stone-50">
    <div class="flex flex-wrap items-center justify-between gap-3 border-b border-stone-200 px-4 py-3">
      <h3 class="min-w-0 [overflow-wrap:anywhere] text-[13px] font-semibold text-stone-900">{{ name }} / SKILL.md</h3>
      <div class="flex shrink-0 items-center gap-2">
        <button type="button" class="button-secondary h-8 px-2.5 text-[11px]" :disabled="translating" @click="toggleTranslation"><LoaderCircle v-if="translating" class="size-3.5 animate-spin" /><Languages v-else class="size-3.5" />{{ translating ? '翻译中' : translatedContent ? (showTranslated ? '查看原文' : '查看中文') : '翻译为中文' }}</button>
        <button type="button" class="button-ghost h-8 px-2 text-[11px]" aria-label="收起 SKILL.md 预览" @click.stop="emit('close')"><ChevronUp class="size-4" />收起预览</button>
      </div>
    </div>
    <p v-if="error" class="border-b border-rose-200 bg-rose-50 px-4 py-2 text-[11px] text-rose-800">{{ error }}</p>
