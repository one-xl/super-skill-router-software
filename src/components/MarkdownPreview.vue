...
<template><article class="max-h-[32rem] min-w-0 max-w-full overflow-y-auto overflow-x-hidden p-5 text-sm leading-6 text-slate-700 [overflow-wrap:anywhere]"><template v-for="(block, index) in blocks" :key="index"><h3 v-if="block.kind === 'heading'" class="mt-5 font-semibold text-slate-950 first:mt-0" :class="block.level === 1 ? 'text-xl' : 'text-base'">{{ block.value }}</h3><pre v-else-if="block.kind === 'code'" class="my-3 max-w-full overflow-x-auto border border-slate-200 bg-slate-50 p-3 text-xs">{{ block.value }}</pre><ul v-else-if="block.kind === 'list'" class="my-3 list-disc space-y-1 pl-5"><li v-for="item in block.value.split('\n')" :key="item">{{ item }}</li></ul><p v-else class="my-3">{{ block.value }}</p></template></article></template>
